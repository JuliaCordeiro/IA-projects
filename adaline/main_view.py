...
def make_window():
    ...
    layout = [[sg.MenubarCustom(menu_def, key='-MENU-')],
              ...
              [sg.Text('Learning Rate', size=10, justification='right',
               ...
               sg.Input(key='-LEARNING_RATE-')],
              [sg.Text('Epochs', size=10, justification='right',
                       tooltip='Max number of epochs to be used in training.'), sg.Input(key='-MAX_EPOCHS-')],
              [sg.Text('Minimum error', size=10, justification='right',
               ...
               sg.Input(key='-MIN_ERROR-')],
              [sg.Button('Train', size=15)],
              [sg.HSeparator()],
              [sg.Column(input_grid_namager), sg.Column(input_selectors_manager, vertical_alignment='top'),
